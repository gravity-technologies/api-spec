...
class GrvtRawAsync(GrvtRawAsyncBase):
    ...
    async def trigger_vault_redemption_v1(
    ...
    async def vault_detail_v1(
        self, req: types.ApiVaultDetailRequest
    ) -> types.ApiVaultDetailResponse | GrvtError:
        resp = await self._post(True, self.td_rpc + "/full/v1/vault_detail", req)
        if resp.get("code"):
            return GrvtError(**resp)
        return from_dict(types.ApiVaultDetailResponse, resp, Config(cast=[Enum]))
